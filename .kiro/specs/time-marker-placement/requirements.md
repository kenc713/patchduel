...
2. The system shall プレイヤーが新たにマーカーを配置せずにターンを終了できることを許容する。ただし、プレイヤーの保有する有効なマーカー数が 0 になることは許容しない（ゲーム開始時に配置される初期マーカーは保持される）。
3. The system shall ゲームを開始するときにプレイヤーを 1 人で初期化できること。
4. When プレイヤーが配置とターン終了を完了したとき, the system shall その行為を記録し、次の操作（UI 上の状態更新）を反映する。
